...
fn composing_engine(reading: &str) -> InputMethodEngine {
    ...
    engine.input_buf.set_text(reading);
...
fn conversion_texts(reading: &str) -> Vec<String> {
    ...
    engine
        .build_conversion_candidates(reading, reading, "", 9, LearningLookup::Exact)
        .into_iter()
        .map(|c| c.text)
        .collect()
}
...
fn rewriter_does_not_expand_dictionary_candidates() {
    ...
    let texts: Vec<String> = engine
        .build_conversion_candidates("てすと", "てすと", "", 9, LearningLookup::Exact)
        .into_iter()
        .map(|c| c.text)
        .collect();
...
fn rewriter_candidates_only_derive_from_user_input() {
    ...
    let candidates = engine.build_conversion_candidates("あ", "あ", "", 9, LearningLookup::Exact);
...
fn alphabet_variants_carry_width_case_descriptions() {
    ...
    let candidates = engine.build_conversion_candidates("abc", "abc", "", 9, LearningLookup::Exact);
...
fn typed_symbol_itself_is_annotated_via_global_lookup() {
    ...
    let candidates = engine.build_conversion_candidates("「", "「", "", 9, LearningLookup::Exact);
