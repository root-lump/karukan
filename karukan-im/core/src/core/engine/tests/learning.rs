...
fn build_candidates_includes_exact_learning() {
    ...
    let texts: Vec<String> = engine
        .build_conversion_candidates("あい", "あい", "", 9, LearningLookup::Exact)
        .into_iter()
        .map(|c| c.text)
        .collect();
...
fn build_candidates_includes_predictions_when_predictive() {
    ...
    let candidates =
        engine.build_conversion_candidates("あい", "あい", "", 9, LearningLookup::Predictive);
...
fn tab_mid_buffer_behaves_like_space() {
    ...
    engine.process_key(&press('a'));
    assert_eq!(engine.input_buf.display(), "あいさ");
    ...
    assert_eq!(engine.input_buf.display(), "あい");
