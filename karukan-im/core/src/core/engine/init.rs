...
impl InputMethodEngine {
    ...
    fn spawn_model_loading(&mut self, settings: &Settings) {
        // `lazy_model_init: false` means "never reach for a model on your
        // own" — the gate test harnesses use so a suite never downloads or
        // loads a GGUF. An explicit `load_converters` call is unaffected.
