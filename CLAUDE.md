...
- **`upstream-sync`** — machine-owned branch maintained by the `upstream-sync` workflow and PR'd into `main`. While it does not yet contain `master`, the workflow force-rebuilds it as `main` + merge of `master`, so anything committed there is discarded on the next run. Once it *does* contain `master` — which is what resolving a merge conflict by hand produces — the workflow stops rebuilding it and only merges `main` into it to keep it current, so a hand-resolved merge survives (and the PR stops being flagged as conflicted). A new `master` commit puts it back under the rebuild rule. On conflict the workflow does not fail: it publishes the conflicting files in the sync PR's body and title, creating that PR on an empty commit if no other head exists.
...
Fork-only deviations from upstream to be aware of when syncing:

- The Space/Tab conversion-role split (`prediction` / `swap_space_tab` in config.toml) intentionally replaces upstream's "predictive candidates on Space, learning-skip on Tab" behavior, and upstream tests asserting that behavior are adapted (predictions surface via the prediction key instead). `LearningLookup` therefore has three variants (`Predictive` / `Exact` / `Skip`) where upstream has two (`Use` / `Skip`); `primary_lookup()` is what upstream's plain "convert normally" call sites map to.
- **During conversion the arrow keys are segment navigation**, not upstream's "dissolve the conversion and move the caret" (togatoga/karukan#112): ← → walk the segments, Shift+← → shrink/expand the conversion range. Upstream's caret-key behavior is kept on Home/End and Ctrl+A/B/E/F, and upstream tests asserting it for ← → are adapted to Home/End.
- **Typing during a conversion depends on live conversion** (`typing_refines()` in `conversion.rs`, which is just `live.enabled`). With live conversion ON, a printable key refines the reading in place and bare digits are ordinary text — upstream's incremental conversion (togatoga/karukan#95) unchanged. With it OFF (manual conversion via Space), a printable key commits the selected candidate through the Enter path and starts the next input, and bare 1–9 select a candidate — mozc's behavior, which is what this fork had before #95. This holds inside the source-filter views too, so manual conversion cannot narrow a view by typing. `Ctrl+1–9` selects in both modes, and Backspace is untouched (it still shrinks the reading inside a filtered view). Upstream tests asserting refine are adapted by turning live conversion on (`enable_refine_on_typing` in `tests/source_filter.rs`, `live.enabled` in `tests/mode_toggle.rs`, `make_live_conversion_engine` in `tests/conversion.rs`); live conversion adds its own ⚡ marker and trailing candidate field to the aux line, so an aux assertion adapted this way checks `contains` where upstream checked `starts_with`.
- F6–F10 character-form conversion (`form.rs`) and the partial conversion at the cursor have no upstream counterpart. Every path that opens a conversion goes through `split_composition_at_caret()`, so the source-filter keys (Ctrl+I / Ctrl+T / Ctrl+R) are bounded by the caret too, where upstream converts the whole reading; upstream tests asserting a single whole-reading commit from a narrowed view are adapted to the two-step partial commit.
...
- **`EngineConfig::lazy_model_init` is the guard, and it only reaches
  ...
  It gates `spawn_model_loading`, the background load `init_from_settings`
  kicks off, so a test suite never downloads or loads a GGUF on its own.
